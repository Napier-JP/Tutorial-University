...
namespace TutorialUniversity.Data
{
    public static class DbInitializer // 初回実行時DBに仮データを入れる Program.csで実行される
    {
        public static void Initialize(SchoolContext context)
        {
            ...
            {
                context.Students.Add(s); // Studentsテーブルに対する操作を登録していく Add(新規追加)/Update(更新)/Remove(削除)がある
            }
            context.SaveChanges(); // これまでに登録された操作を実行
            ...
            {
            new Penalty{StudentID=2,IssueDate=DateTime.Parse("2005-04-01"),Charge="Public Intoxication"}
            };
